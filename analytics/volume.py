...
def muscle_group_summary(weeks: int = 8) -> dict[str, float]:
    ...
    Averages over the requested window (see weeks_denominator) — averaging only
    the weeks a muscle was trained would inflate sparse training."""
    df = weekly_volume(weeks)
    if df.empty:
        return {}
    denom = weeks_denominator(weeks)
    avg = (df.groupby("muscle")["volume_kg"].sum() / denom).round(1)

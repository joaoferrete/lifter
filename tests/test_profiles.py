...
def test_create_profile_creates_directory(tmp_path):
    from profiles import create_profile, PROFILES_DIR
...
def test_create_profile_sets_active_if_none():
    from profiles import create_profile, get_active_slug
    p = create_profile("Solo")
    assert get_active_slug() == p["slug"]


def test_create_profile_does_not_override_existing_active():
    from profiles import create_profile, get_active_slug, set_active_slug
...
def test_get_active_slug_round_trip():
    from profiles import create_profile, set_active_slug, get_active_slug
...
def test_activate_profile_updates_config_db_path(tmp_path):
    import config
    from profiles import create_profile, activate_profile, PROFILES_DIR
    p = create_profile("Alice")
...
def test_activate_profile_updates_hevy_api_key():
    import config
    from profiles import create_profile, activate_profile
...
def test_rename_profile_updates_name(tmp_path):
    from profiles import create_profile, rename_profile, list_profiles, PROFILES_DIR
...
def test_delete_profile_removes_directory(tmp_path):
    from profiles import create_profile, delete_profile, PROFILES_DIR
...
def test_delete_active_profile_reassigns_active():
    from profiles import create_profile, delete_profile, set_active_slug, get_active_slug
...
def test_get_profile_name_returns_name():
    from profiles import create_profile, get_profile_name
...
def test_get_profile_name_returns_slug_when_not_found():
    from profiles import get_profile_name
...
def test_update_profile_key(tmp_path):
    from profiles import create_profile, update_profile_key, PROFILES_DIR
    p = create_profile("Alice")
